...
st.sidebar.title("⚙️ Mode Aplikasi")
app_mode = st.sidebar.radio(
    "Pilih Fitur:",
    ["📊 Hitung Bagi Hasil", "📄 Cetak Slip Gaji (dari Excel)"]
)
...
#  SLIP BAGI HASIL PDF
# ---------------------------------------------------------------------------
LOGO_MADINAH = Path(__file__).parent / 'assets' / 'logo-madinah.png'
LOGO_MFLASH = Path(__file__).parent / 'assets' / 'logo-mflash.png'

SLIP_POTONGAN = [
    'Potongan Kasbon', 'Potongan Refund', 'Potongan AR',
    'Potongan Terlambat', 'Potongan Minus Audit',
    'Potongan Audit Compliance', 'Potongan Koperasi',
]

# Mapping kolom RAW → label slip potongan
_MAP_POTONGAN_RAW = {
    ...
    'Potongan Koperasi': 'Biaya Pendaftaran Koperasi',
}


def _rp_slip(v):
    """Format angka ke Rp x.xxx.xxx tanpa singkatan."""
    try:
        v = float(v)
    except (TypeError, ValueError):
        return 'Rp 0'
...
def _gambar_slip(c, data, page_w, page_h):
    """Menggambar 1 halaman slip bagi hasil pada canvas `c`.

    `data` adalah dict berisi:
        nama, cabang, periode_teks,
        kualifikasi: list of (label, omzet, tarif_frac, bagi_hasil),
        potongan: dict label→nominal,
    """
    margin_x = 51
    content_w = page_w - 2 * margin_x
    y = page_h - 40  # mulai dari atas

    # --- Warna ---
    BIRU_GELAP = HexColor('#1F3864')
    HIJAU_MUDA = HexColor('#DBEBDA')
    ABU = HexColor('#B3B3B3')
    ...
    if LOGO_MADINAH.exists():
        try:
            c.drawImage(str(LOGO_MADINAH), margin_x, y - logo_h, width=logo_h, height=logo_h,
                        preserveAspectRatio=True, mask='auto')
        except Exception:
            pass
    if LOGO_MFLASH.exists():
        try:
            mflash_w = logo_h * (1400 / 990)  # aspect ratio dari gambar asli
            c.drawImage(str(LOGO_MFLASH), page_w - margin_x - mflash_w, y - logo_h,
                        width=mflash_w, height=logo_h,
                        preserveAspectRatio=True, mask='auto')
        except Exception:
            pass
    ...
    c.drawCentredString(page_w / 2, y, 'SLIP BAGI HASIL TEKNISI MADINAH FLASH')
    y -= 14
    ...
    info_items = [
        ('Nama', f": {data['nama']}"),
        ('Jabatan', ': Teknisi'),
        ('Divisi', f": MFlash \u2014 {data['cabang']}"),
        ('Periode', f": {data['periode_teks']}"),
    ]
    for label, val in info_items:
        c.setFont('Helvetica-Bold', 9.5)
        c.drawString(margin_x, y, label)
        c.setFont('Helvetica', 9.5)
        c.drawString(119, y, val)
        y -= 15.5

    y -= 14

    # ===== TABEL PENDAPATAN PER KUALIFIKASI =====
    # Header bar biru gelap
    header_h = 16
    c.setFillColor(BIRU_GELAP)
    c.rect(margin_x, y - header_h, content_w, header_h, fill=1, stroke=0)
    c.setFillColor(white)
    c.setFont('Helvetica-Bold', 8.5)
    c.drawString(margin_x + 6, y - header_h + 4.5, 'PENDAPATAN PER KUALIFIKASI')
    c.drawString(margin_x + content_w * 0.63, y - header_h + 4.5, 'OMZET')
    c.drawString(margin_x + content_w * 0.77, y - header_h + 4.5, 'AKAD')
    c.drawString(margin_x + content_w * 0.87, y - header_h + 4.5, 'BAGI HASIL')
    y -= header_h + 3

    # Baris kualifikasi
    c.setFillColor(black)
    total_bruto = 0.0
    for label, omzet, tarif_frac, bh in data['kualifikasi']:
        c.setFont('Helvetica', 9)
        c.drawString(margin_x + 6, y - 11, label)
        c.drawRightString(margin_x + content_w * 0.72, y - 11, _rp_slip(omzet))
        c.drawString(margin_x + content_w * 0.77, y - 11, _pct_slip(tarif_frac))
        c.drawRightString(margin_x + content_w - 4, y - 11, _rp_slip(bh))
        total_bruto += float(bh) if not pd.isna(bh) else 0
        y -= 15

    y -= 5

    # Garis di atas Total Bruto
    col_total_x = margin_x + content_w * 0.70
    c.setStrokeColor(black)
    c.setLineWidth(0.6)
    c.line(col_total_x, y, page_w - margin_x, y)
    y -= 15

    # Total Bruto Bagi Hasil
    c.setFont('Helvetica-Bold', 9.5)
    c.drawString(margin_x + 6, y, 'Total Bruto Bagi Hasil')
    c.drawRightString(margin_x + content_w - 4, y, _rp_slip(total_bruto))
    y -= 28
    ...
    c.setFillColor(white)
    c.setFont('Helvetica-Bold', 8.5)
    c.drawString(margin_x + 6, y - header_h + 4.5, 'POTONGAN')
    c.drawRightString(margin_x + content_w - 4, y - header_h + 4.5, 'JUMLAH')
    y -= header_h + 3

    c.setFillColor(black)
    total_potongan = 0.0
    for label in SLIP_POTONGAN:
        val = data['potongan'].get(label, 0)
        if pd.isna(val):
            val = 0
        total_potongan += float(val)
        c.setFont('Helvetica', 9)
        c.drawString(margin_x + 6, y - 11, label)
        c.drawRightString(margin_x + content_w - 4, y - 11, _rp_slip(val))
        y -= 15.5

    y -= 3

    # Garis di atas Total Potongan
    c.setLineWidth(0.6)
    c.line(col_total_x, y, page_w - margin_x, y)
    y -= 15
    ...
    # Total Potongan
    c.setFont('Helvetica-Bold', 9.5)
    c.drawString(margin_x + 6, y, 'Total Potongan')
    c.drawRightString(margin_x + content_w - 4, y, _rp_slip(total_potongan))
    y -= 22

    # ===== NETT BAGI HASIL (background hijau muda) =====
    nett = total_bruto - total_potongan
    nett_h = 22
    c.setFillColor(HIJAU_MUDA)
    c.rect(margin_x, y - nett_h, content_w, nett_h, fill=1, stroke=0)
    c.setFillColor(black)
    c.setFont('Helvetica-Bold', 11)
    c.drawString(margin_x + 6, y - nett_h + 6, 'NETT BAGI HASIL')
    c.drawRightString(margin_x + content_w - 4, y - nett_h + 6, _rp_slip(nett))
    y -= nett_h + 18

    # ===== CATATAN =====
    c.setFont('Helvetica-Bold', 8.5)
    c.drawString(margin_x, y, 'Catatan')
    y -= 8
    box_h = 57
    c.setStrokeColor(ABU)
    c.setLineWidth(0.6)
    c.rect(margin_x, y - box_h, content_w, box_h, fill=0, stroke=1)
    y -= box_h + 28

    # ===== TANDA TANGAN =====
    ttd_labels = ['Teknisi', 'Kepala Cabang', 'Finance']
    ttd_positions = [
        margin_x + content_w * 0.10,
        margin_x + content_w * 0.43,
        margin_x + content_w * 0.77,
    ]
    line_w = 90
    c.setStrokeColor(black)
    c.setLineWidth(0.6)
    for pos, lbl in zip(ttd_positions, ttd_labels):
        lx = pos - line_w / 2 + 15
        c.line(lx, y + 12, lx + line_w, y + 12)
        c.setFont('Helvetica', 8.5)
        c.drawCentredString(pos + 15, y, lbl)


def baca_excel_slip(file_bytes):
    """Membaca file Excel hasil download yang sudah diisi manual (potongan dll)
    dan mengembalikan list of dict berisi data teknisi untuk dicetak ke PDF.
    """
    wb = openpyxl.load_workbook(io.BytesIO(file_bytes), data_only=True)
    hasil = []
    
    for sheet_name in wb.sheetnames:
        if sheet_name.lower().strip() in ['rincian faktur penjualan', 'pivot', 'final']:
            continue
        ws = wb[sheet_name]
        
        # Ekstrak periode dari sel A2 (format: "Periode: X · Tarif: Y")
        val_a2 = str(ws.cell(row=2, column=1).value or '')
        periode_teks = ''
        if 'Periode:' in val_a2:
            try:
                part = val_a2.split('Periode:')[1]
                periode_teks = part.split('·')[0].strip()
            except Exception:
                pass
                
        # Baca Header di row 4
        headers = {}
        for c in range(1, ws.max_column + 1):
            val = str(ws.cell(row=4, column=c).value or '').strip()
            if val:
                headers[val] = c
                
        # Baca Data mulai row 5
        for r in range(5, ws.max_row + 1):
            col_nama = headers.get('Nama Teknisi', 1)
            nama_tek = str(ws.cell(row=r, column=col_nama).value or '').strip()
            
            # Berhenti jika ketemu TOTAL atau kosong
            if not nama_tek or nama_tek.upper() == 'TOTAL' or nama_tek.upper() == 'NONE':
                continue
                
            cabang = str(ws.cell(row=r, column=headers.get('Cabang', 2)).value or '').strip()
            
            # Susun kualifikasi
            kualifikasi = []
            for kat in KATEGORI_ORDER:
                col_omzet = headers.get(f'Omzet {kat}')
                col_bh = headers.get(f'Bagi Hasil {kat}')
                
                try: omzet_val = float(ws.cell(row=r, column=col_omzet).value or 0) if col_omzet else 0.0
                except: omzet_val = 0.0
                
                try: bh_val = float(ws.cell(row=r, column=col_bh).value or 0) if col_bh else 0.0
                except: bh_val = 0.0
                
                if omzet_val == 0 and bh_val == 0:
                    continue
                    
                tarif_frac = (bh_val / omzet_val) if omzet_val != 0 else 0.0
                kualifikasi.append((kat, omzet_val, tarif_frac, bh_val))
            ...
            # Susun potongan
            potongan = {}
            for slip_lbl, raw_col in _MAP_POTONGAN_RAW.items():
                col_idx = headers.get(raw_col)
                try: val = float(ws.cell(row=r, column=col_idx).value or 0) if col_idx else 0.0
                except: val = 0.0
                potongan[slip_lbl] = val
...
def buat_slip_pdf_bytes_v2(data_list):
    """Generate 1 PDF multi-page dari list of dict data teknisi."""
    buf = io.BytesIO()
    page_w, page_h = A4

    c = rl_canvas.Canvas(buf, pagesize=A4)
    c.setTitle('Slip Bagi Hasil Teknisi')

    if not data_list:
        c.setFont('Helvetica', 14)
        c.drawCentredString(page_w / 2, page_h / 2, 'Tidak ada data teknisi yang valid.')
        c.save()
        buf.seek(0)
        return buf.getvalue()
    ...
    for idx, data in enumerate(data_list):
        if idx > 0:
            c.showPage()
        _gambar_slip(c, data, page_w, page_h)
...
def buat_slip_zip_v2(data_list):
    """Generate ZIP berisi 1 PDF per teknisi dari list of dict data teknisi."""
    zip_buf = io.BytesIO()
    page_w, page_h = A4
    ...
    with zipfile.ZipFile(zip_buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        for data in data_list:
            ...
            cv.setTitle(f"Slip Bagi Hasil - {data['nama']}")
            _gambar_slip(cv, data, page_w, page_h)
            cv.save()
            pdf_buf.seek(0)

            safe_name = re.sub(r'[\\/:*?"<>|]', '_', f"{data['cabang']} - {data['nama']}")
    ...
    st.sidebar.divider()
    st.sidebar.title("📄 File Excel Bagi Hasil")
    st.sidebar.info("Upload file Excel Bagi Hasil yang sudah Anda **isi manual potongannya** di sini.")
    up_slip = st.sidebar.file_uploader(
        "Upload Excel terisi",
        type=['xlsx'],
        accept_multiple_files=False,
        key='uploader_slip_sidebar'
    )

    st.title("📄 Cetak Slip Bagi Hasil (PDF)")
    
    if up_slip is not None:
        try:
            data_slip = baca_excel_slip(up_slip.getvalue())
            if not data_slip:
                st.warning("Tidak ditemukan data teknisi di dalam Excel yang diupload (pastikan Anda mengupload file yang benar).")
            else:
                st.success(f"Berhasil membaca data untuk **{len(data_slip)} teknisi** dari Excel.")
                
                col_pdf1, col_pdf2 = st.columns(2)
                with col_pdf1:
                    if st.button("📑 Cetak PDF Gabungan", key='btn_pdf_v2', use_container_width=True):
                        with st.spinner("Menyusun PDF Gabungan..."):
                            st.session_state['slip_pdf_bytes_v2'] = buat_slip_pdf_bytes_v2(data_slip)
                            st.session_state['slip_pdf_filename_v2'] = f"Slip Bagi Hasil.pdf"
                    if st.session_state.get('slip_pdf_bytes_v2') is not None:
                        st.download_button(
                            "⬇️ Unduh PDF Gabungan",
                            data=st.session_state['slip_pdf_bytes_v2'],
                            file_name=st.session_state.get('slip_pdf_filename_v2', 'Slip.pdf'),
                            mime='application/pdf',
                            use_container_width=True, key='dl_pdf_v2')
                            
                with col_pdf2:
                    if st.button("📦 Cetak ZIP (per teknisi)", key='btn_zip_v2', use_container_width=True):
                        with st.spinner("Menyusun file ZIP..."):
                            st.session_state['slip_zip_bytes_v2'] = buat_slip_zip_v2(data_slip)
                            st.session_state['slip_zip_filename_v2'] = f"Slip Bagi Hasil.zip"
                    if st.session_state.get('slip_zip_bytes_v2') is not None:
                        st.download_button(
                            "⬇️ Unduh ZIP",
                            data=st.session_state['slip_zip_bytes_v2'],
                            file_name=st.session_state.get('slip_zip_filename_v2', 'Slip.zip'),
                            mime='application/zip',
                            use_container_width=True, key='dl_zip_v2')
        except Exception as e:
            st.error(f"Terjadi kesalahan saat membaca Excel: {e}")
    else:
        st.info("👈 Silakan upload file Excel melalui panel di sebelah kiri untuk mulai mencetak slip.")
    
    st.stop()


st.sidebar.divider()
st.sidebar.title("📁 Sumber Data Mentah")
...
nama_file_slip_pdf = f"Slip Bagi Hasil {label_tgl_file} {nama_cabang_file}.pdf"
nama_file_slip_zip = f"Slip Bagi Hasil {label_tgl_file} {nama_cabang_file}.zip"
